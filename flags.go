...
func (flags *Flags) GetToken() string {
	return flags.Lookup("token").Value.String()
}
...
func (flags *Flags) IsNoContext() bool {
	return flags.Lookup("no_context").Value.String() == "true"
}
...
func (flags *Flags) SetParams(context whisper.Context) error {
	if lang := flags.GetLanguage(); lang != "" {
	...
	if flags.IsNoContext() {
		fmt.Fprintf(flags.Output(), "Setting no_context to true\n")
		context.SetNoContext(true)
	}
...
func registerFlags(flag *Flags) {
	...
	flag.String("language", "auto", "Spoken language")
	flag.Bool("no_context", true, "do not use past transcription (if any) as initial prompt for the decoder")
